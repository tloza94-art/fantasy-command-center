const API="https://api.sleeper.app/v1",SEASON="2026";
let S={user:null,leagues:[],rows:[],players:{},nflState:{},lineups:[],matchups:{}};
...
async function load(){
  ...
  try{
    ...
    S.user=user;
    [S.leagues,S.players,S.nflState]=await Promise.all([
...
function renderHome(){
  let starters=S.rows.filter(r=>r.rosterStatus==="Starter"),m=starters.filter(must),w=starters.filter(watch),map={};
  starters.filter(injured).forEach(r=>{if(!map[r.id])map[r.id]={...r,leagues:[]};map[r.id].leagues.push(r.league)});
  ...
  $("mustFixList").innerHTML=m.length?m.map(card).join(""):'<div class="muted">✅ No must-change injured starters.</div>';
  $("watchList").innerHTML=w.length?w.map(card).join(""):'<div class="muted">✅ No Q/D starters.</div>';
  $("riskList").innerHTML=risks.length?risks.map(r=>riskCard(r)).join(""):'<div class="muted">✅ No multi-league injury risks.</div>';
}

function renderSunday(){
  let week=Number(S.nflState.display_week||S.nflState.week||1);
  let starters=S.rows.filter(r=>r.rosterStatus==="Starter"),m=starters.filter(must),w=starters.filter(watch);
  ...
  $("sundaySummary").textContent=affected.size?affected.size+" league"+(affected.size===1?"":"s")+" need attention across your portfolio.":"Every loaded lineup currently looks clean.";
  $("sundayAllClear").innerHTML=!urgentCount&&!w.length?'<div class="all-clear-card">✅ <strong>All clear.</strong> No empty slots, OUT/IR/PUP/SUSP starters, or Q/D starters detected.</div>':"";
  ...
  $("sundayUrgentList").innerHTML=urgentCards.length?urgentCards.join(""):'<div class="muted">✅ Nothing urgent detected.</div>';
  $("sundayWatchList").innerHTML=w.length?w.map(r=>sundayPlayerCard(r,"MONITOR")).join(""):'<div class="muted">✅ No questionable or doubtful starters.</div>';
  $("sundayRiskList").innerHTML=risks.length?risks.map(r=>riskCard(r,true)).join(""):'<div class="muted">✅ No injured starter is hitting multiple leagues.</div>';
